...
<?php
	if(isset($_POST['delete'])){
		...
		if($query){
				...
				echo "alert('Xóa Bài Thành Công');";
				echo "</script>";
		}else{
				echo "<script type='text/javascript'>";
				echo "alert('Xóa Bài Thất Bại');";
				echo "</script>";
		}
	}
?>
...
<div id="main">
	<div id="main-left">
	 <div id="list">
	 	<div id="list-title">Bài Viết</div> 
	 </div>
		<?php
			...
			$sql="SELECT posts.id,posts.title,posts.content,posts.excerpt,posts.user,posts.datetime,posts.cat_id,category.id as cid ,category.title as name FROM posts,category  WHERE category.id=posts.cat_id ORDER BY posts.id DESC LIMIT $startfrom,$postpage";
		...
		<div class="main-content">
			...
			<div class="main-left-content">
					...
					<div class="main-node">
						<font color="red">Date: </font><?php echo $row["datetime"]; ?><br>
						<!--xoa bai viet-->
						<form method="post">
				...
				<ul>
					...
					<?php if($totalpage > 0) : ?>
		                <?php for($i=0;$i<$totalpage;$i++): ?>
		                   ...
		                   href="posts.php?page=<?php echo ($i+1); ?>"><?php echo $i+1;?></a>
		                <?php endfor; ?>
		              <?php endif; ?>	
